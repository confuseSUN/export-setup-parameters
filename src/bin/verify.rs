...
fn main() {
    ...
    println!("Read G2 elements: {}", g2.len()); // Read G2 elements: 2
    assert_eq!(g2.len(), 2);
    ...
    println!("Read G1 elements: {}", g1.len()); // Read G1 elements: 2097155
    ...
    let accumulated_g1 =
        ark_ec::msm::VariableBase::msm(&g1[..g1.len() - 1], &rand[0..g1.len() - 1]);
    let accumulated_g1_shifted =
        ark_ec::msm::VariableBase::msm(&g1[1..g1.len()], &rand[0..g1.len() - 1]);

    let miller_loop_result = ark_bls12_381::Bls12_381::miller_loop(&[
        (accumulated_g1.into_affine().into(), g2[1].into()),
        (
            accumulated_g1_shifted.neg().into_affine().into(),
            g2[0].into(),
        ),
    ]);

    let pairing_result =
        ark_bls12_381::Bls12_381::final_exponentiation(&miller_loop_result).unwrap();
